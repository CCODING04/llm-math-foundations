...
其中 $w_i$ 是词汇表中的第 $i$ 个 token，这个概率分布满足 $\sum_{i=1}^{V} P(w_i \mid x_{<t}) = 1$（$V$ 是词汇表大小；作为参考，GPT-2 的词汇表大小约为 50,257，LLaMA 约为 32,000）。
...
> 💡 **argmax** 表示"使后面表达式取最大值的参数值"。$x_t = \arg\max_{w_i} P(w_i)$ 意思就是"让 $P$ 最大的那个 $w_i$"。
...
> 💡 **logits（逻辑值）**：模型最后一层输出的、未经 Softmax 归一化的原始分数。它可以是任意实数（正数、负数、很大、很小）。Softmax 的作用就是把这些"分数"转化为概率分布。可以理解为 logits 是"投票数"，Softmax 把投票数变成"投票比例"。
...
## 🔢 公式推导：温度 Softmax 的梯度（⭐ 进阶选读，跳过不影响后续阅读）
...
在聊 RLHF 之前，先简单交代一下背景：**RLHF（Reinforcement Learning from Human Feedback，基于人类反馈的强化学习）** 的核心思路是——让人类给模型的不同回答打分（哪个好、哪个差），然后用这些分数作为"奖励信号"来训练模型，让模型逐步学会生成更符合人类偏好的回答。具体细节咱们第九章会详细展开，这里只需要知道它是一种"用人类偏好来微调模型"的方法就行。
