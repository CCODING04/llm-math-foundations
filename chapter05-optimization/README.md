...
> 💡 **符号小课堂**：$\nabla$ 读作 **nabla**（纳布 拉），$\nabla_\theta L(\theta)$ 表示损失函数对参数 $\theta$ 的**梯度**——也就是由各个参数的偏导数组成的向量。
> 打个比方：你站在山坡上，梯度 $\nabla_\theta L$ 就像告诉你"哪个方向坡度最陡"的指示牌。梯度指向函数值增长最快的方向，所以我们要**反方向**走（减去梯度），才能下山。
...
> 💡 **大O记号**：$O(f(n))$ 表示"当 $n$ 很大时，最多是 $f(n)$ 的常数倍"。用于描述算法复杂度的增长趋势。例如 $O(n^2)$ 意味着计算量随 $n$ 的平方增长，$O(1/t)$ 意味着误差随步数 $t$ 线性减小。
...
设 $\beta_1 = 0.9$，假设前三步的梯度为 $g_1 = 2, g_2 = 3, g_3 = 1$：

| 步数 $t$ | 当前梯度 $g_t$ | 计算 $m_t = 0.9 \times m_{t-1} + 0.1 \times g_t$ | 结果 $m_t$ |
|-----------|----------------|------------------------------------------------------|------------|
| 1 | $g_1 = 2$ | $0.9 \times 0 + 0.1 \times 2$ | $0.2$ |
| 2 | $g_2 = 3$ | $0.9 \times 0.2 + 0.1 \times 3$ | $0.48$ |
| 3 | $g_3 = 1$ | $0.9 \times 0.48 + 0.1 \times 1$ | $0.532$ |

可以看到：虽然 $m_0 = 0$ 导致初期值偏小，但**历史梯度在逐渐累积**——$m_t$ 越来越接近梯度的真实水平。$\beta_1$ 越大（接近 1），"记忆"越长，更新越平滑。

> 🎯 同理，二阶矩 $v_t$ 做的是完全一样的事，只不过是对 $g_t^2$ 做指数加权平均。
...
**🔢 偏差修正效果一览（$\beta_1 = 0.9$，接上面的数字演示）**

| $t$ | $m_t$（修正前） | $1 - \beta_1^t$ | $\hat{m}_t = m_t / (1 - \beta_1^t)$（修正后） |
|-----|------------------|------------------|---------------------------------------------|
| 1 | 0.2 | $1 - 0.9 = 0.1$ | $0.2 / 0.1 = 2.0$ |
| 2 | 0.48 | $1 - 0.81 = 0.19$ | $0.48 / 0.19 ≈ 2.53$ |
| 3 | 0.532 | $1 - 0.729 = 0.271$ | $0.532 / 0.271 ≈ 1.96$ |

> 🎯 **观察**：修正前 $m_t$ 被 $m_0 = 0$ 严重拉低（如 $m_1 = 0.2$ 远小于真实梯度 2）。修正后 $\hat{m}_t$ 立刻回到合理范围（如 $\hat{m}_1 = 2.0$ 正好等于 $g_1$）。随着 $t$ 增大，$1 - \beta_1^t \to 1$，修正自动消失——这就是偏差修正的精妙之处！
...
> 🎯 **什么是凸函数？想象一个碗。** 碗的形状有什么特点？——不管你在碗边选哪两个点，连接它们的线段都**不会掉到碗里面去**（都在碗壁上方或碗壁上）。这就是凸函数的核心特征。

现在看数学定义。一个函数 $f$ 是**凸函数**，如果对任意 $x, y$ 和 $\lambda \in [0, 1]$：

$$f(\lambda x + (1-\lambda)y) \leq \lambda f(x) + (1-\lambda)f(y)$$

这里 $\lambda \in [0, 1]$ 是什么意思？——它表示**在 $x$ 和 $y$ 之间的一个插值点**。$\lambda x + (1-\lambda)y$ 就是从 $x$（$\lambda=0$）到 $y$（$\lambda=1$）连线上的某个位置。不等式左边是函数在这个插值点的值，右边是两端函数值的加权平均。

> 💡 不等式说的就是：**函数图像上任意两点的连线，都在函数图像上方。** 这正是"碗形"的数学描述。

**🔢 用一个简单例子验证**：取 $f(x) = x^2$（抛物线），选 $x = 0, y = 2$，令 $\lambda = 0.5$：
> - 左边：$f(0.5 \times 0 + 0.5 \times 2) = f(1) = 1^2 = 1$
> - 右边：$0.5 \times f(0) + 0.5 \times f(2) = 0.5 \times 0 + 0.5 \times 4 = 2$
> - $1 \leq 2$ ✅ 不等式成立！$f(x) = x^2$ 确实是凸的。
