...
> 🤔 **为什么等概率时熵最大？** 想象你要猜一个骰子的结果。如果知道某面概率 90%，你几乎不需要信息就能猜对（确定性高→熵低）。但如果每面概率都是 1/6，你完全无法预判（不确定性最高→熵最大）。**最大熵原理就是：在已知信息之外，做最少的额外假设。**
...
>
> 💡 **one-hot（独热编码）**：一种向量表示方法，只有对应真实类别的那一维为 1，其余为 0。例如词汇表有 5 个词，真实词是第 2 个，则 one-hot 向量为 [0, 1, 0, 0, 0]。这代表"我 100% 确定是这个 token"的分布。
>
...
假设 $X$ = 天气（晴天 / 雨天），$Y$ = 是否带伞（带伞 / 不带伞），联合分布如下：

|  | 带伞 $Y\!=\!0$ | 不带伞 $Y\!=\!1$ | $P(X)$ |
|--|---------------|----------------|-------|
| 晴天 $X\!=\!0$ | 0.10 | 0.40 | 0.50 |
| 雨天 $X\!=\!1$ | 0.40 | 0.10 | 0.50 |
| $P(Y)$ | 0.50 | 0.50 | 1.00 |

计算：
- $H(X) = -0.5\log_2 0.5 - 0.5\log_2 0.5 = 1.0$ bit
- $H(Y) = 1.0$ bit（同理）
- $H(Y|X) = 0.5 \times H(Y|X\!=\!0) + 0.5 \times H(Y|X\!=\!1)$
  - 晴天时：$P(Y|X\!=\!0) = [0.2, 0.8]$，$H = -0.2\log_2 0.2 - 0.8\log_2 0.8 \approx 0.72$ bit
  - 雨天时：$P(Y|X\!=\!1) = [0.8, 0.2]$，$H \approx 0.72$ bit
  - $H(Y|X) = 0.5 \times 0.72 + 0.5 \times 0.72 = 0.72$ bit
- $I(X; Y) = H(Y) - H(Y|X) = 1.0 - 0.72 = 0.28$ bit

**直觉：** 知道天气之后，关于"是否带伞"的不确定性从 1.0 bit 降到了 0.72 bit——减少了 0.28 bit，这就是互信息。天气和带伞之间确实有关联，但不是完全确定（晴天也有人带伞，雨天也有人忘带），所以互信息不是 1.0 bit。
...
> 💡 **前置知识：Jensen 不等式**
>
> 对于凸函数 $f$，有：**凸函数在平均值处的值 ≤ 各值处函数的平均值**，即 $f(\mathbb{E}[X]) \leq \mathbb{E}[f(X)]$。
>
> **一个简单例子**：$f(x) = x^2$ 是凸函数。
> - $f((1+3)/2) = f(2) = 4$
> - $(f(1) + f(3))/2 = (1 + 9)/2 = 5$
> - 所以 $4 < 5$ ✅，符合 Jensen 不等式。
>
> 直觉：凸函数"向上弯"，所以取平均后再算函数值，会比先算函数值再取平均要小。
...
> 📌 "这里用了 Jensen 不等式：把求和看作期望 $\mathbb{E}[f(X)]$，其中 $f(t) = -\log t$，$X = q(x)/p(x)$，概率为 $p(x)$。根据 Jensen 不等式，$\mathbb{E}[f(X)] \geq f(\mathbb{E}[X])$。"
...
# Softmax 转换为概率
# 💡 Softmax 的含义：把任意实数向量转换为概率分布（所有值在 0~1 之间，总和为 1）。
# 公式：softmax(x_i) = exp(x_i) / sum(exp(x_j))，数值越大→概率越高。
# 它是 one-hot 的"软"版本——one-hot 是 100% 确定，softmax 给出概率化的"确定性程度"。
def softmax(x):
    e_x = np.exp(x - np.max(x))  # 数值稳定：减去最大值防止溢出
    return e_x / e_x.sum()
