...
> 💬 **两个学派，一场对话**
>
> **频率学派**：「概率就是长期频率。抛硬币一万次，正面朝上的比例就是概率。它客观存在，不因你的想法改变。」
>
> **贝叶斯学派**：「那如果只能抛三次呢？概率也可以是我根据已有信息对某件事的信心程度——先相信什么，看到数据后再更新信念。」
>
> **核心分歧**：概率到底是世界的固有属性（频率派），还是我们对世界的不确定性度量（贝叶斯派）？在机器学习中，频率派思路催生了 MLE（本章第 6 节），贝叶斯派思路催生了贝叶斯推断和贝叶斯神经网络。两者各有用武之地，不必选边站。
...
> 💡 **为什么重要？** 语言模型预测一个句子的概率，用的就是链式法则：$P(w_1, w_2, \ldots, w_n) = P(w_1) \cdot P(w_2|w_1) \cdot P(w_3|w_1,w_2) \cdots P(w_n|w_{<n})$。先把乘法公式记住，后面会反复用到。
...
其中 $n! = n \times (n-1) \times \cdots \times 1$（读作「n 的阶乘」，例如 $5! = 5 \times 4 \times 3 \times 2 \times 1 = 120$），$\binom{n}{k} = \frac{n!}{k!(n-k)!}$ 是组合数——从 n 个中选 k 个有多少种选法。
...
> 🌱 **先来一个直觉**：量身高。你的身高精确到 170.000…cm 的概率是多少？答案是 **0**——因为「精确等于某个值」对连续量来说几乎不可能。但你落在 169～171cm 之间的概率显然是正的！连续随机变量就是这样：**单个点的概率是 0，但一个区间的概率是正的**。所以我们不问「P(X = 某值)」，而问「P(X 落在某区间)」。
...
**交叉熵损失**（分类任务常用）则和信息论挂钩（下一章讲），它的本质是**负对数似然的期望**。
...
> 🗣️ **人话版**：你掷骰子掷得越多，平均点数就越接近 3.5。不是「可能接近」，而是数学上保证它会收敛——试验次数足够多，样本均值就锁定了真实期望。

**弱大数定律**（切比雪夫形式）：设 $X_1, X_2, \ldots$ 是**独立同分布**（i.i.d.，即每个变量互不影响，且都来自同一个概率模型）的随机变量，均值为 μ，则对任意 ε > 0：
...
> 🗣️ **人话版**：不管你原始数据长什么样——偏的、扁的、双峰的——只要你抽足够多个样本算平均值，这些平均值的分布都会变成钟形（正态分布）。这就是大自然最神奇的「批量抹平」效应。
...
> 💡 **求导提示**：这里用到的是复合函数求导（链式法则）。比如 $\frac{d}{dp}[\log p] = \frac{1}{p}$，以及 $\frac{d}{dp}[\log(1-p)] = \frac{-1}{1-p}$（外层 log，内层 1-p，别忘了对内层再求导乘上去）。
...
📌 关于 Beta 分布的简要说明：
Beta 分布是 [0,1] 区间上的连续概率分布，常用作概率之概率的模型。
它的参数 (a, b) 可以直观理解：a = 曾看到的「成功」次数 + 1，b = 曾看到的「失败」次数 + 1。
当 a=b=1 时，Beta(1,1) 就是均匀分布——表示「我对 p 一无所知」。
观察到 k 次成功、m 次失败后，后验就变成 Beta(a+k, b+m)。
这就是为什么 Beta 分布是伯努利/二项分布的「天然共轭先验」——更新规则特别简洁。
"""
import numpy as np
import matplotlib.pyplot as plt
...
| Softmax | 将 logits（模型输出的原始分数，还没归一化的实数）转换为概率分布 |
