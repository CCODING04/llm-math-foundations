...
学习本章前，你需要熟悉：
- **Ch5 优化理论**：梯度下降和目标函数优化的基本思想（策略梯度就是在优化一个目标函数）
- **Ch3 概率论**：条件概率、期望的计算（策略 $\pi_\theta(a|s)$ 就是条件概率）
- **Ch6 信息论**：KL 散度的定义和含义（PPO 和 DPO 都依赖 KL 约束）
...
> **核心**：策略梯度是所有 RL 对齐方法的基础——"奖励高就增大该动作概率"这个直觉，贯穿 PPO、DPO、GRPO。
...
> **深入**：GRPO 的完整推导涉及 PPO 的裁剪机制和组内标准化——理解其核心思想（"组内对比代替绝对评分"）是关键，数学细节可按需深入。
...
> **核心**：DPO 的核心思想是"绕过奖励模型，直接用偏好数据训练"——理解配分函数消去这一步，就理解了 DPO 为什么能简化 RLHF。
...
> **深入**：配分函数 $Z(x)$ 的消去是 DPO 推导中最关键的一步——它让我们不需要计算难以处理的归一化常数，就能从偏好数据中直接学习。
...
> **核心**：PPO 是 RLHF 中最经典的方法——理解裁剪机制（clip）是掌握 PPO 的关键，它防止策略更新太大导致训练不稳定。
...
1. **解释策略梯度的直觉**：说清"奖励高→增大该动作概率，奖励低→降低概率"的核心思想
2. **理解 PPO 的裁剪机制**：解释 clip 的作用是防止策略更新太大（"一步走太远摔下山坡"）
3. **对比 DPO 和 RLHF**：说清 DPO 为什么不需要单独训练奖励模型，以及它的数学等价性
4. **解释 GRPO 的核心思想**：理解"组内相对排名代替绝对奖励"的思路和优势
5. **理解配分函数消去**：解释 DPO 推导中 $Z(x)$ 如何在偏好差中被消掉
...
PPO 的裁剪机制看起来很技术化，但它解决的是一个很实际的问题：**防止策略更新太大**。

类比：你在山坡上找最低点。如果每步迈得太大，可能直接跨过谷底甚至翻到另一座山上——这就是"策略崩溃"。PPO 的 clip 就像在脚上绑了绳子，每步最多走 $[1-\epsilon, 1+\epsilon]$ 这么远，保证平稳下山。

> 💡 具体来说：概率比 $r_t(\theta) = \pi_\theta / \pi_{\text{old}}$ 衡量了策略变化幅度。如果 $r_t$ 跑到 $[1-\epsilon, 1+\epsilon]$ 之外，clip 就把它截住——"你已经走够远了，停下来看看再说。"

### 卡点 2：DPO 为什么不需要 reward model？

传统 RLHF 需要：① 训练奖励模型 → ② 用 PPO 优化策略。DPO 跳过了 ①，直接从偏好数据学习。

**数学上的等价性**：DPO 推导的关键一步是，从 KL 约束的最优策略中反解出 reward 函数：
$$r(x,y) = \beta \ln \frac{\pi^*(y|x)}{\pi_{\text{ref}}(y|x)} + \text{const}$$
然后代入 Bradley-Terry 偏好模型时，**配分函数 $Z(x)$ 被消掉了**（因为它在 $r(y_w) - r(y_l)$ 的差中抵消）。所以 DPO 可以直接用策略网络的 log 概率代替 reward，不需要显式训练奖励模型。

> 💡 一句话：DPO 把"训练奖励模型 + 优化策略"两步合并成了一步——直接用偏好数据训练策略，数学上等价但工程上简单得多。
